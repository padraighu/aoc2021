    ...
    for line in lines:
        ...
        if diagonal:
            if delta_x * delta_y > 0:
                for x, y in zip(range(min(a[0], b[0]), max(a[0], b[0])+1), range(min(a[1], b[1]), max(a[1], b[1])+1)):
                    cover[(x, y)] += 1
            else:
                for x, y in zip(range(min(a[0], b[0]), max(a[0], b[0])+1), range(max(a[1], b[1]), min(a[1], b[1])-1, -1)):
                    cover[(x, y)] += 1
    for cnt in cover.values():
        if cnt > 1:
            res += 1
